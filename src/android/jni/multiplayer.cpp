    ...
    if (auto room = Network::GetRoomMember().lock()) {
        ...
        if (!members.empty()) {
            ...
            for (const auto& member : members) {
                infolist.push_back((member.nickname.empty() ? "Player" : member.nickname) + "|" + (member.game_info.name.empty() ? room->GetRoomInformation().preferred_game : member.game_info.name));
            }
        }
    }
    return infolist;
